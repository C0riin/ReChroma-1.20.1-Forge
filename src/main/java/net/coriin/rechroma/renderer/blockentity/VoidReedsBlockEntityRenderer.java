...
public class VoidReedsBlockEntityRenderer implements BlockEntityRenderer<VoidReedsBlockEntity> {
    ...
    @Override
    public void render(VoidReedsBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        if(ReChromaHelper.canSee(Minecraft.getInstance().player)) {
            renderSingleBlock(ModBlocks.VOID_REEDS.get().defaultBlockState(),
    ...
    public void renderSingleBlock(BlockState state, PoseStack p_110914_, MultiBufferSource p_110915_,
                                  int pPackedLight, int p_110917_, ModelData modelData, RenderType renderType) {
    ...
    @Override
    public boolean shouldRenderOffScreen(VoidReedsBlockEntity pBlockEntity) {
        return true;
    }
}
