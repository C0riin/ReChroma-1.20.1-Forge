...
public class CastingTableMenu extends AbstractContainerMenu {
    ...
    protected CastingTableMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(10));
    }

    public CastingTableMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            for (int j = 0; j < 3; j++) {
                for (int i = 0; i < 3; i++) {
                    this.addSlot(new SlotItemHandler(handler, j*3+i, 62 + 18*i, 37 + j*18));
                }
            }
